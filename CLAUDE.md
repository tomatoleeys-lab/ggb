...
- **`demoMode`일 때는 DB에 쓰지 않는다**: `?demo=1`은 화면 확인용이므로 추가·삭제·예산 저장 모두 메모리에서만 처리한다.
...
DB 관련 코드를 만졌으면 **내역 추가 → 새로고침 → 그대로 남아 있는지**, 삭제·예산 저장까지 실제로 눌러서 확인한다. 콘솔 에러와 상태 배지도 함께 본다.
